...
public class ExperienceDaoImpl implements ExperienceDao {
    ...
    public List<ExperienceModel> listAll() {
        return new ArrayList<>(jdbcTemplate.query(
                "SELECT experienceId, experienceName, address, description, siteUrl, price," +
                        " cityId, categoryId, userId, hasImage FROM experiences",
                EXPERIENCE_MODEL_ROW_MAPPER));
    }

    @Override
    public Optional<ExperienceModel> getById(long experienceId) {
        return jdbcTemplate.query("SELECT experienceId, experienceName, address, description," +
                        " siteUrl, price, cityId, categoryId, userId, hasImage FROM experiences WHERE experienceId = ?",
    ...
    public List<ExperienceModel> listByCategory(long categoryId) {
        return jdbcTemplate.query("SELECT experienceId, experienceName, address, description, siteUrl," +
                        " price, cityId, categoryId, userId, hasImage" +
                        " FROM experiences NATURAL JOIN categories WHERE categoryId = ?",
    ...
    public List<ExperienceModel> listByCategoryAndCity(long categoryId, long cityId) {
        return jdbcTemplate.query("SELECT experienceId, experienceName, address, description, siteUrl," +
                        " price, cityId, categoryId, userId, hasImage" +
                        "FROM experiences NATURAL JOIN categories WHERE categoryId = ? AND cityId = ?",
