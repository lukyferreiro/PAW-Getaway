...
public class JwtUtil {
    ...
    public UserDetails decodeToken(String token) {
        try {
            ...
            final Collection<GrantedAuthority> authorities = getAuthorities(claims.get("roles", String.class));
    ...
    public String generateToken(UserModel userDetails, String baseUrl) {
        Claims claims = Jwts.claims();

        claims.put("roles", serializeRoles(userDetails.getRoles()));
        claims.put("userUrl", baseUrl + "api/users/" + userDetails.getUserId());
    ...
    private String serializeRoles(Collection<RoleModel> roles) {
        final StringBuilder sr = new StringBuilder();

        for (RoleModel role : roles) {
            sr.append(role.getRoleName().name()).append(' ');
        }
